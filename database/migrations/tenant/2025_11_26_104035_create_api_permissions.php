...
return new class extends Migration
{
    ...
    public function up(): void
    {
        $roles_structure = [
            ...
            'user' => [
                'users' => 'v',
                'groups' => 'c,vo,uo,do',
                'file_or_folders' => 'c,vo,uo,do,fdo,ro',
                'tenant_contracts' => 'v',
                'contacts' => 'c,vo,uo,do',
                'links' => 'c,vo,uo,do',
                'histories' => 'v',
    ...
    protected function createRoles(array $roles_structure): void
    {
        foreach ($roles_structure as $role_name => $permissions) {
            ...
            Role::create(['name' => $role_name, 'guard_name' => 'api']);
        }
    }
    ...
    protected function createPermissions(array $roles_structure, array $except): void
    {
        ...
        foreach ($roles_structure as $role_name => $permissions) {
            // Find the role with 'guard_name' => 'api'
        ...
        foreach ($roles_structure as $role_name => $permissions) {
            ...
            foreach ($permissions as $entity_name => $permission_values) {
                ...
                foreach (explode(',', $permission_values) as $permission_value) {
                    // Ensure 'api' guard when revoking permission
                    $role->revokePermissionTo(Permission::findByName($this->formatPermissionName($permissions_map_filtered[$permission_value], $entity_name), 'api'));
                }
            }
        }
    }
    ...
    {
        foreach (explode(',', $permission_values) as $permission_value) {
            // Ensure 'api' guard when assigning permission
            $role->givePermissionTo(Permission::findByName($this->formatPermissionName($permissions_map_filtered[$permission_value], $entity_name), 'api'));
        }
    }
    ...
    private function createPermissionsIfDoesntExist(array $permissions_map_filtered, int|string $entity_name): void
    {
        foreach ($permissions_map_filtered as $permission_map) {
            ...
            // Si la permission n'existe pas déjà, la créer avec 'api' guard
