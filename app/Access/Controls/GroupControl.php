        ...
        return [
            GlobalPerimeter::new()
                ->allowed(function (Model $user, string $method) {
                    return $user->can(sprintf('%s_groups', $method));
                })
                ->should(function () {
                    return true;
                })
            ...
            OwnPerimeter::new()
                ->allowed(function (User $user, string $method) {
                    return $user->can(sprintf('%s_own_groups', $method));
                })
                ->should(function (User $user, Group $group) {
                    return $group->users()->where('id', $user->id)->exists();
                })
